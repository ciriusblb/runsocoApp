...
export default function Pedidos({navigation}){
  ...
  const selectedComponent = (key, msj) => {
    switch (key) {
  ...
  useEffect(() => {
    async function fetchIdSale() {
        try {
          await AsyncStorage.getItem('idSale').then(
            value =>{
              setIdSale(value)
            }
          )
  ...
  const confirmarPedido = async () => {
    try {
      ...
      setIsLoading(true)
      const res = await axios.get(`${API}/sale/confirmation/${idSale}`, {
          headers: {
      ...
      setIsLoading(false)
      selectedComponent("ConfirmationText", 'Hola mundo')
    } catch (e) {
      setIsLoading(false)
        console.log(e);
    }
